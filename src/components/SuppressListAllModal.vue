<template>
  <div class="fixed inset-0 z-50 flex items-center justify-center">
    ...
    ></div>
    <form @submit.prevent="submitSuppressionListAll()">
      <div class="relative bg-white rounded-2xl shadow-2xl max-w-lg w-full opacity-100">
        <div class="flex items-center justify-between p-4 md:p-5 rounded-t border-gray-200">
          <div class="flex gap-8 items-start">
            ...
            <div>
              <div class="font-semibold text-gray-900 text-2xl mb-4">
                Confirm deletion of all lists
              </div>
              <div class="text-base text-gray-900">
                Are you sure you want to delete all the {{ listStore.totalCountList - 1 }} lists?
              </div>
...
async function submitSuppressionListAll() {
  try {
    for (var i = 0; i < listStore.lists.length; i++) {
      if (listStore.lists[i]?.id != 1) {
        if (listStore.lists[i]?.count != 0) {
         await postStore.updatePostCategory(listStore.lists[i]?.id)
          await listStore.suppressList(listStore.lists[i]?.id)
          // listStore.deleteListStore(listStore.lists[i]?.id)
        } else {
          await listStore.suppressList(listStore.lists[i]?.id)
          // listStore.deleteListStore(listStore.lists[i]?.id)
        }
    ...
    emit('close')
    toast.success(`Deletion of all lists successful `, {
      timeout: 2000,
    })
  } catch (error) {
    toast.error(error)
