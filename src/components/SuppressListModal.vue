<template>
    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <form
        @submit.prevent="
          submitSuppressionList(listStore.listSuppress?.id, listStore.listSuppress?.name)
        "
      >
        <div class="relative bg-white rounded-2xl shadow-2xl max-w-lg w-full opacity-100">
            ...
            <div class="flex gap-8 items-start">
              ...
              <div>
                ...
                <div class="text-base text-gray-900">
                  Are you sure you want to delete this list {{ listStore.listSuppress?.name }} ?
                </div>
          ...
          <div class="p-6">
            <div class="flex gap-6 w-full justify-end">
              <button
                type="button"
                @click="($emit('close'), listStore.clearSuppressList())"
...
async function submitSuppressionList(idList, nameList) {
  try {
    listStore.deleteListStore(idList)
    listStore.clearSuppressList()
    await listStore.suppressList(idList)
    await postStore.updatePostCategory(idList)
    emit('close')
    toast.success(`List deletion ${nameList} `, {
      timeout: 2000,
    })
  } catch (error) {
    toast.error(error)
